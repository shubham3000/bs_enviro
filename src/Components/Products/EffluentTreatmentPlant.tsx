...
const EffluentTreatmentPlant: React.FC = () => {
  const [tabIndex, setTabIndex] = useState(0);
  const tabSectionRef = useRef<HTMLDivElement | null>(null);

  const handleNext = () => {
    if (tabIndex < effluentTreatmentPlant.length - 1) {
            ...
            <TabPanel key={tab.id}>
              ...
              <div className="flex justify-between items-center mt-12">
                {index > 0 ? (
                  ...
                  >
                    <FaArrowLeftLong/> Previous: {effluentTreatmentPlant[index - 1].label}
                  </button>
                ) : (
                  <div />
                )}

                {index < effluentTreatmentPlant.length - 1 && (
                  <button
                    onClick={handleNext}
                  ...
                  >
                    Next: {effluentTreatmentPlant[index + 1].label} <FaArrowRightLong/>
                  </button>
                )}
              </div>
            </TabPanel>
