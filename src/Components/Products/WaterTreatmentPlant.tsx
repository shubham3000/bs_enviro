...
const WaterTreatmentPlant: React.FC = () => {
    const [tabIndex, setTabIndex] = useState(0);
    const tabSectionRef = useRef<HTMLDivElement | null>(null);

    const handleNext = () => {
        if (tabIndex < waterTreatmentPlant.length - 1) {
            ...
            <div className="container mx-auto">
                <Tabs selectedIndex={tabIndex} onSelect={setTabIndex}>
                    <TabList className="flex border-b border-gray-300 mb-6 overflow-x-auto no-scrollbar mt-2 lg:mt-6 w-full">
                        {waterTreatmentPlant.map((tab: TabData) => (
                            <Tab
                                key={tab.id}
                    ...
                    {waterTreatmentPlant.map((tab: TabData, index: number) => (
                        <TabPanel key={tab.id}>
                            <div className="space-y-8 mt-8">
                                ...
                                <motion.h2
                                    className="text-3xl whitespace-normal break-words w-full md:text-5xl xl:text-6xl text-[#0195B1] font-semibold font-montserrat mt-12"
                                    {...textAnimationProps}
                                >
                                    {tab.title}
                                </motion.h2>
                                ...
                                {tab.sideImage ? (
                                    <div className="w-full flex flex-col-reverse gap-12 md:flex-row">
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-12 w-full">
                                ...
                                {index < waterTreatmentPlant.length - 1 && (
                                    <button
                                        onClick={handleNext}
