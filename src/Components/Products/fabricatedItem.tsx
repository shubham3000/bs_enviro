...
import { fabricatedItem } from "@/data/fabricatedItems";
...
const FabricatedItem: React.FC = () => {
  const [tabIndex, setTabIndex] = useState(0);
  const tabSectionRef = useRef<HTMLDivElement | null>(null);

  const handleNext = () => {
    if (tabIndex < fabricatedItem.length - 1) {
      ...
      <div className="container mx-auto">
        <Tabs selectedIndex={tabIndex} onSelect={setTabIndex}>
          <TabList className="flex border-b border-gray-300 mb-6 overflow-x-auto no-scrollbar mt-2 lg:mt-6 w-full">
            {fabricatedItem.map((tab: TabData) => (
              <Tab
                key={tab.id}
          ...
          {fabricatedItem.map((tab: TabData, index: number) => (
            <TabPanel key={tab.id}>
              ...
              <div className="flex flex-col md:flex-row justify-between items-center gap-4 mt-12 w-full">
                {index > 0 ? (
                  ...
                  >
                    ...
                    {fabricatedItem[index - 1].label}
                  </button>
                ) : (
                  <div className="w-full md:w-auto" />
                )}

                {index < fabricatedItem.length - 1 && (
                  <button
                    onClick={handleNext}
                  ...
                  >
                    Next: {fabricatedItem[index + 1].label}{" "}
                    <FaArrowRightLong />
                  </button>
                )}
              </div>
